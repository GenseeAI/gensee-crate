...
impl SqliteStore {
    ...
    /// Security burst cannot grow the local database without bound. Other
    /// collectors enforce their caps with source-specific retention jobs.
    ...
    pub fn system_events_for_sources(
        ...
        let path_predicate = if let Some(path) = path_contains {
            parameters.push(rusqlite::types::Value::Text(path.to_string()));
            "AND instr(COALESCE(args, ''), ?) > 0"
        } else {
            ""
        };
        parameters.extend([
...
mod tests {
    ...
    fn system_events_sort_by_numeric_timestamp() {
        ...
        let source_timestamps = store
            .system_events_for_sources(&["macos-eslogger"], None, None, i64::MIN, i64::MAX, 100)
            .unwrap()
            .into_iter()
            .map(|event| event.ts)
            .collect::<Vec<_>>();
        ...
        assert!(store
            .system_events_for_sources(&["linux-falco"], None, None, i64::MIN, i64::MAX, 100)
            .unwrap()
            .is_empty());
        assert!(store
            .system_events_for_sources(&[], None, None, i64::MIN, i64::MAX, 100)
            .unwrap()
            .is_empty());

        assert_eq!(
            store
                .system_events_for_sources(&["macos-eslogger"], None, None, 900, 1500, 2)
                .unwrap()
                .into_iter()
                .map(|event| event.ts)
                .collect::<Vec<_>>(),
        ...
        assert_eq!(
            store
                .system_events_for_sources(&["macos-eslogger"], None, None, i64::MIN, i64::MAX, 2,)
                .unwrap()
                .into_iter()
                .map(|event| event.ts)
                .collect::<Vec<_>>(),
    ...
    #[test]
    fn endpoint_retention_does_not_consume_the_falco_cap() {
        let path = std::env::temp_dir().join(format!(
        ...
        let pruned = store
            ...
            .unwrap();
        assert_eq!(pruned.system_events, 0);
        assert_eq!(
            store
                .system_events_for_sources(&["linux-falco"], None, None, i64::MIN, i64::MAX, 100)
                .unwrap()
        ...
        assert_eq!(
            store
                .prune_system_event_source_retention("linux-falco", 0, 2, 100)
                .unwrap(),
            3
        );
