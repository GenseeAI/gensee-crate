...
pub(crate) fn tclone_context_run_id() -> Option<String> {
    read_tclone_run_context()
        .ok()?
        .get("run_id")?
        .as_str()
        .map(str::trim)
...
pub(crate) fn tclone_fork(args: Vec<OsString>) -> io::Result<()> {
    ...
    let result: io::Result<()> = (|| {
        ...
        let clone = run_tclone_clone_with_overlay_retry(
            &podman,
            copies,
            &prefix,
            &source,
...
impl TclonePreparedForkContexts {
    fn commit(&mut self) {
        self.committed = true;
    }
}

impl Drop for TclonePreparedForkContexts {
    fn drop(&mut self) {
        for context in &self.contexts {
            let _ = fs::remove_file(&context.payload_path);
            if !self.committed {
                let _ = revoke_tclone_host_control_capability(&context.run_id);
            }
        }
        if let Some(parent) = self
            .contexts
            .first()
            .and_then(|context| context.payload_path.parent())
        {
...
fn prepare_tclone_fork_contexts(
    source: &TcloneRunRecord,
    forked_at_ms: u64,
    copies: usize,
) -> io::Result<TclonePreparedForkContexts> {
    let context_dir = gensee_tmp_root()?
        .join(&source.run_id)
        .join("prepared-fork-contexts");
    create_restrictive_dir_all(&context_dir)?;
    let mut prepared = TclonePreparedForkContexts {
        contexts: Vec::with_capacity(copies),
        committed: false,
    };
...
fn run_tclone_clone_attempts(
    ...
    match run_command_capture_with_env(podman, &clone_args, &env) {
        ...
        Err(error) if use_overlay && should_retry_tclone_without_overlay(&error.to_string()) => {
            ...
            let fallback_args = tclone_clone_args(
                copies,
                &fallback_prefix,
                &source.container_name,
                false,
...
fn retry_tclone_partial_multicopy_clone(
    ...
    let retry_args = tclone_clone_args(
        copies,
        &retry_prefix,
        &source.container_name,
        context.use_overlay,
...
fn validate_tclone_clone_output(output: &str, copies: usize) -> io::Result<()> {
    let clones = parse_tclone_fork_clone_metadata(output)?;
    if clones.len() != copies {
        return Err(io::Error::other(format!(
            "podman returned {} cloned container metadata record(s), expected {copies}",
            clones.len()
...
fn tclone_clone_args(
    ...
    for (index, context) in prepared_contexts.iter().enumerate() {
        args.push(OsString::from(format!(
            "--tfork-inject-file={}:{}:{}",
            index,
            context.payload_path.to_string_lossy(),
...
fn parse_tclone_network(value: &str) -> io::Result<Option<String>> {
        ...
        .all(|byte| byte.is_ascii_alphanumeric() || matches!(byte, b'-' | b'_' | b'.' | b':'))
    {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
                "{TCLONE_NETWORK_ENV} contains unsupported characters: {value}"
            ),
        ));
    }
    ...
    for raw in value.split(',') {
        ...
        if !valid_start || !bytes.all(|byte| byte.is_ascii_alphanumeric() || byte == b'_') {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                format!(
                    "{TCLONE_FORWARD_ENV_ENV} contains invalid environment name: {name}"
                ),
            ));
        }
...
mod tests {
    ...
    #[test]
    fn tclone_clone_args_preinstall_each_fork_context() {
        let contexts = vec![
            TclonePreparedForkContext {
                run_id: "fork-0".to_string(),
                payload_path: PathBuf::from("/tmp/context-0.json"),
            },
            TclonePreparedForkContext {
                run_id: "fork-1".to_string(),
    ...
    fn tclone_prepared_context_commit_preserves_capability() -> io::Result<()> {
        let _guard = tclone_test_env_lock();
        let run_id = format!("commit-prepared-{}", Uuid::new_v4().simple());
        let run_root = gensee_tmp_root()?.join(&run_id);
        let payload_dir = run_root.join("prepared-fork-contexts");
        create_restrictive_dir_all(&payload_dir)?;
        let payload_path = payload_dir.join("context.json");
    ...
    fn tclone_partial_multicopy_retry_detects_crun_short_clone() {
        ...
        assert!(validate_tclone_clone_output(
            r#"[{"id":"a","name":"fork-0","pid":1,"rootfs":"/tmp/0"}]"#,
            2
        )
        .is_err());
        assert!(validate_tclone_clone_output(
            r#"[{"id":"a","name":"fork-0","pid":1,"rootfs":"/tmp/0"},{"id":"b","name":"fork-1","pid":2,"rootfs":"/tmp/1"}]"#,
            2
        )
        .is_ok());
    }
...
"#;
    ...
    fn tclone_runtime_options_parse_network_and_forwarded_environment() {
        ...
        assert_eq!(parse_tclone_network("bridge:lab_1").unwrap(), Some("bridge:lab_1".into()));
        assert_eq!(parse_tclone_network("  ").unwrap(), None);
        assert!(parse_tclone_network("host --privileged").is_err());

        assert_eq!(
            parse_tclone_forward_env_names(
                "LITELLM_API_KEY, ARTIFACTORY_URL,LITELLM_API_KEY"
            )
            .unwrap(),
