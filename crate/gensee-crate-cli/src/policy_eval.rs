...
pub(crate) fn fork_suggestion_finding(
    ...
    let message = if let Some(run_id) = current_run_id.filter(|run_id| !run_id.trim().is_empty()) {
        format!(
            "This looks suitable for a forked run ({reason}); do not run it in the source container. Ask the user to approve a forked run. If approved, run this once: gensee run fork {run_id} --name {name_hint} --attach tmux:right --json. The live-cloned Codex turn continues the original task automatically in the fork; do not resend the prompt with `gensee run send`. If the fork command returns scheduled=true, do not run fork again and do not wait for the source to become idle. Immediately poll the returned status_command and retry that same command while status=running. The active poll is intentionally inherited by the live clone, which will stop source orchestration and continue the approved task. If status=failed, stop and report it. If the source receives status=succeeded, the fork is already continuing independently; do not resend the prompt or perform the task locally. The fork will summarize its own changed files and tests, ask whether to merge, keep working, or discard, and wait for explicit user approval before running the selected lifecycle command internally. Do not auto-merge and do not ask the user to type Gensee lifecycle commands.",
            reason = reason.label()
        )
    } else {
        format!(
