...
pub(crate) fn fork_suggestion_finding(
    ...
    let message = if let Some(run_id) = current_run_id.filter(|run_id| !run_id.trim().is_empty()) {
        format!(
            "This looks suitable for a forked run ({reason}); do not run it in the source container. Ask the user to approve a forked run. If approved, run this once: gensee run fork {run_id} --name {name_hint} --attach tmux:right --json. If the fork command returns scheduled=true, do not run fork again, do not poll fork-status, and do not perform the task locally. End this source turn normally so Gensee can clone the idle Codex session; Gensee will submit the saved original request to the fork automatically and open its pane. The fork will summarize its own changed files and tests, ask whether to merge, keep working, or discard, and wait for explicit user approval before running the selected lifecycle command internally. Do not auto-merge, do not resend the prompt with `gensee run send`, and do not ask the user to type Gensee lifecycle commands.",
            reason = reason.label()
        )
    } else {
        format!(
