...
pub(crate) fn fork_suggestion_finding(
    ...
    let message = if let Some(run_id) = current_run_id.filter(|run_id| !run_id.trim().is_empty()) {
        format!(
            "This looks suitable for a forked run ({reason}); do not run it in the source container. Ask the user to approve a forked run. If approved, run this once: gensee run fork {run_id} --name {name_hint} --attach tmux:right --json. The live-cloned Codex turn continues the original task automatically in the fork; do not resend the prompt with `gensee run send`. If the fork command returns scheduled=true, do not run fork again; pause without running source-container shell commands such as `sleep`, then poll the returned status_command until status=succeeded, or stop and report the failure if status=failed. Shell wait commands can be captured by the live clone and make the source uncloneable. If a poll returns status=running with retry_after_ms or transient=true, pause without shell commands before retrying that same status command; never create another fork for the job. Once status=succeeded, use forks[0].run_id and poll `gensee run list --json` until that fork has task_status=completed. Then run `gensee run summary <fork-id> --json` and present the changed files and tests in chat. Do not auto-merge and do not ask the user to type Gensee lifecycle commands. Offer three choices: merge the changes back, keep working in the fork, or discard the fork. Wait for explicit user approval before running `gensee run merge`, `gensee run switch`, or `gensee run discard` internally.",
            reason = reason.label()
        )
    } else {
        format!(
...
fn tclone_fork_command_finding(
    ...
    let message = if duplicate {
        format!(
            "A fork for source {source_run_id}{} was already scheduled in this session. Do not run gensee run fork again and do not resend the original prompt. The live-cloned Codex turn continues the task automatically in the fork. In the source, poll the previous fork-status command if available, otherwise poll gensee run list until the fork appears.",
            name.as_deref()
